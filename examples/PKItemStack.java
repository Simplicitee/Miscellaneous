...
public class PKItemStack {
	...
	public static void checkInstances() {
		...
		for (ItemStack stack : instances.keySet()) {
			...
			PKItemStack pkStack = instances.get(stack);
			if (pkStack == null) {
				toRemove.add(stack);
				continue;
			}
			
			Player owner = Bukkit.getPlayer(pkStack.getOwner());
			if (owner == null) {
				toRemove.add(stack);
				continue;
			}
	...
	public static PKItemStack loadFromItemStack(ItemStack itemstack) {
		...
		if (instances.containsKey(itemstack)) {
			return instances.get(itemstack);
		}
